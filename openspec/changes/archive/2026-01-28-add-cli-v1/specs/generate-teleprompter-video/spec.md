...
The system SHALL offer interactive style options for aspect ratio, background color, font, and font weight.

#### Scenario: Aspect ratio selection
- **WHEN** the user selects an export aspect ratio
- **THEN** the video is rendered at 4:5 or 1:1 dimensions suitable for Instagram

#### Scenario: Background color selection
- **WHEN** the user selects a background color
- **THEN** the rendered video uses the chosen color

#### Scenario: Font selection
- **WHEN** the user selects a font and font weight
- **THEN** the rendered text uses the selected font styling
...
The system SHALL generate an ElevenLabs voiceover from the input text and render a teleprompter-style MP4 video with word-by-word highlighting synced to the audio.

#### Scenario: Word-by-word highlight
- **WHEN** the video is rendered
- **THEN** each word is highlighted in order as it is spoken in the audio
